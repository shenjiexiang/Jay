...
{
    //手部动画
    public GameObject handAnimatiom;
    Animator handAnimator;
    public bool m_beenGrap;
    //常规参数
    public GameObject lightAk;
	...
	void Start()
	{
        ...
        //手部动画
        handAnimator = handAnimatiom.GetComponent<Animator>();
        m_beenGrap = false;
	}
	...
	void Update() 
	{
        //手部动画
        if (m_beenGrap == false)
            handAnimator.SetBool("beenGrap", false);
        if (m_beenGrap == true )
            handAnimator.SetBool("beenGrap", true );
					...
					if(Physics.Raycast(ray, out hit))
					{
						foreach(GameObject obj in draggableObjects)
						{
                            ...
                            if (hit.collider.gameObject == obj)
							{
                                ...
                                //抓住后旋转
                                draggedObject.transform.Rotate(Vector3.down, 85 * Time.deltaTime * 5);
